...
import { AuditLog, PageResponse } from '@/lib/types';
import PaginationControls from '@/components/ui/PaginationControls';
import { X } from 'lucide-react';

export default function AuditLogsPage() {
    const [page, setPage] = useState(0);
    const { data, isLoading } = useSWR<PageResponse<AuditLog>>(
        `/api/audit?page=${page}&size=10&sort=createdAt,desc`,
        fetcher
    );
            ...
            <div className="p-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="bg-[#1a1426] rounded-2xl border border-slate-800 p-2 mb-6 flex items-center gap-4">
                    ...
                    <div className="flex gap-2 pr-2">
                        <button className="px-4 py-3 bg-[#111827] hover:bg-slate-800 rounded-xl text-sm font-semibold text-slate-400 border border-slate-800" type="button">
                            User: All
                        </button>
                        <button className="px-4 py-3 bg-[#111827] hover:bg-slate-800 rounded-xl text-sm font-semibold text-slate-400 border border-slate-800" type="button">
                            Action: All
                            ...
                            <div key={log.id} className="p-6 hover:bg-slate-800/30 transition-all group">
                                <div className="flex items-start justify-between gap-6">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <div className="bg-[#111827] rounded-xl border border-slate-800 p-4 font-mono text-xs flex flex-col gap-2">
                                                <div className="flex items-center gap-2 text-slate-500">
                                                    ...
                                                    <span>Request IP: {String(log.metadata?.ip || 'Unknown')}</span>
                                                    <span className="mx-1">•</span>
                                                    <span>Environment: {String(log.metadata?.env || 'Unknown')}</span>
                                                </div>
                                                {log.metadata && (
                                    ...
                                    <div className="flex flex-col items-end gap-4">
                                        ...
                                        <button
                                            className="flex items-center gap-2 text-sm font-bold text-primary opacity-0 group-hover:opacity-100 transition-all hover:bg-primary/10 px-4 py-2 rounded-xl"
            ...
            {selectedLog && (
                <>
                    <button
                        ...
                        type="button"
                    />
                    <div className="fixed inset-x-0 top-20 z-50 mx-auto w-[92%] max-w-3xl rounded-2xl border border-slate-700 bg-[#111827] p-6 shadow-2xl">
                        ...
                        <div className="space-y-3 text-sm">
                            <p className="text-slate-300">
                                <span className="font-bold text-slate-100">User:</span>{' '}
                            ...
                            <p className="text-slate-300">
                                <span className="font-bold text-slate-100">Environment:</span>{' '}
                                {String(selectedLog.metadata?.env || 'Unknown')}
                            </p>
                            <div>
                                <p className="mb-2 font-bold text-slate-100">Metadata</p>
                                <pre className="max-h-72 overflow-auto rounded-xl border border-slate-800 bg-slate-900/80 p-3 text-xs text-slate-300">
